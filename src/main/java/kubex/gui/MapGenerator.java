...
public class MapGenerator 
{
				...
				for(int cz=0;cz<Chunk.CHUNK_DIMENSION;cz++)
				{
					...
					if(mapcode<3)ret[cx][cy][cz]=getCubeFromHeightMap(cx,cy+(y*Chunk.CHUNK_DIMENSION),cz);
						...
						if(this.mapcode==5 && cy + y*Chunk.CHUNK_DIMENSION<=SEA_HEIGHT+15){
							...
							if(ret[cx][cy][cz]==1 && cy + y*Chunk.CHUNK_DIMENSION < SEA_HEIGHT+5&&Math.random()<0.01)  ret[cx][cy][cz]=12; //Generate some lights randomly
						}
					}
					
					if(cy==0&&y==0) ret[cx][cy][cz]=27; //Indestructible block on bottom of the map, to prevent player from falling. It can still be destroyed using flying mode.
	...
	{
		boolean testGroundOnly=false;
		if(c.isConstantValue()){ //If all chunk cubes are the same, if the chunk is empty we will only check its ground layer. If not, there is not possibility of generation, we simply return.
			if(c.getCubeAt(0, 0, 0)==0) testGroundOnly=true;
			else return;
		}
		
		int herbylimit=this.mapcode<3?SEA_HEIGHT+1-c.getY()*Chunk.CHUNK_DIMENSION : this.mapcode==5? SEA_HEIGHT+16-c.getY()*Chunk.CHUNK_DIMENSION : -1;
		...
		for(int x=0;x<Chunk.CHUNK_DIMENSION;x++)
		{
			for(int z=0;z<Chunk.CHUNK_DIMENSION;z++)
			{
				...
				for(int y=0;y<(testGroundOnly?1:Chunk.CHUNK_DIMENSION);y++)
				{
					byte cube=c.getCubeAt(x, y, z);
					if(cube==0&&lastCube==1){
						double rand=Math.random();
						if(Math.random()<0.005) generateTreeAt(x,y,z,c);
						//Generate some vegetation randomly on top of the grass. Vegetation need some light to grow!
						else if(y>herbylimit&&rand<0.12&&c.getNaturalLightLevelAt(x, y, z)>(byte)5){  
							if(rand<0.1) c.setCubeAt(x, y, z, (byte)19);
							else c.setCubeAt(x, y, z, (byte)21);
						}
					}
					lastCube=cube;
				}
	...
	private void generateTreeAt(int x,int y,int z,Chunk c)
	{
		...
		for(int cy=0;cy<height;cy++)
		{
			...
			if(Math.random()*(cy/(float)(height))>0.3f){ //Grows one branch if random god says so
